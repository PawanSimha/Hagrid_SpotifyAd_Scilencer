document.addEventListener('DOMContentLoaded', () => {
    ...
    // Helper to run code only if element exists
    ...
    // ── WAVE ANIMATION ──
    safeInit('phoneWave', (phoneCanvas) => {
    ...
    // Phone mute counter
    safeInit('muteCount', (el) => {
        let muteNum = 47;
        setInterval(() => {
            if (Math.random() > 0.7) {
                muteNum++;
                el.innerHTML = `${muteNum} <span>↑</span>`;
            }
        }, 3000);
    });

    // ── DEMO WAVE ──
    safeInit('waveCanvas', (demoCanvas) => {
    ...
    function animateAuditCard(card) {
        ...
        if (!scoreText || !ringActive) return;
    ...
    function animateCounters() {
        document.querySelectorAll('[data-target]').forEach(el => {
            ...
            const suffix = target === 18 ? ' MB' : target === 3 ? '' : '';
            let start = 0;
    ...
    if (nav) {
        window.addEventListener('scroll', () => {
            nav.style.boxShadow = window.scrollY > 40 ?
                '0 8px 32px rgba(28,25,23,0.1), 0 1px 0 rgba(255,255,255,0.8) inset' :
                '0 4px 24px rgba(28,25,23,0.06), 0 1px 0 rgba(255,255,255,0.8) inset';
            updateActiveNav();
        });
    }
    updateActiveNav();
